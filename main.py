...
def team_power_rating(team):
    ...
    return (win_percentage * 30) + (point_differential * 1.5) + (streak * 15) # win percentage is weighed more
...
def away_rating(team):
    ...
    point_differential = team["ppg"] - team ["oppg"]
    streak = sum(team["last5"]) / 5
    away_factor = away_factor(team)
    return ((win_percentage * 30) + (point_differential * away_factor * 1.5) + (streak * 15))

home_court_advantage = 3.0

def calc_spread(home_team, away_team):
    home_rating = team_power_rating(home_team)
    away_rating = away_rating(away_team)
