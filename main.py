...
def main():
    ...
    while True:
        ...
        if choice == "1": 
            home_team = input("Enter home team abbreviation (e.g. OKC): ").upper()
            away_team = input("Enter away team abbreviation (e.g. SAS): ").upper()
            if home_team not in TEAMS or away_team not in TEAMS:
                print("Invalid team abbreviation(s), please try again.")
                continue
            if home_team == away_team:
                print("Home and away teams cannot be the same, please try again.")
                continue
